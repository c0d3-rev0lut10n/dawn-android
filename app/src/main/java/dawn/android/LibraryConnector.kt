...
data class GenInitRequest(
    ...
    val own_pubkey_curve: String? = null,
    val own_seckey_curve: String? = null,
    val own_pubkey_curve_pfs: String? = null,
    val own_seckey_curve_pfs: String? = null,
...
data class ParseInitRequest(
    ...
    val remote_pubkey_curve: String?,
    val remote_pubkey_curve_pfs: String?,
...
@Serializable
data class AcceptInitRequest(
    val status: String,
    val new_pfs_key: String?,
    val own_pubkey_kyber: String?,
    val own_seckey_kyber: String?,
    val mdc: String?,
...
object LibraryConnector {
    ...
    fun mGetNextTimestamp(timestamp: String): Result<Timestamp, String> {
        val libraryResponse: Timestamp = Json.decodeFromString(getNextTimestamp(timestamp))
        if(libraryResponse.status != "ok") return err(libraryResponse.status)
        return ok(libraryResponse)
    }
    ...
    fun mGetAllTimestampsSince(timestamp: UInt): Result<MultiTimestamp, String> {
        val convertedTimestamp = timestamp.toString()
        val libraryResponse: MultiTimestamp = Json.decodeFromString(getAllTimestampsSince(convertedTimestamp))
        if(libraryResponse.status != "ok") return err(libraryResponse.status)
        return ok(libraryResponse)
    }
    ...
    fun mGenHandle(init_pubkey_kyber: String, init_pubkey_curve: String, init_pubkey_curve_pfs_2: String, init_pubkey_kyber_for_salt: String, init_pubkey_curve_for_salt: String, name: String, mdc: String): Result<GenHandle, String> {
        val libraryResponse: GenHandle = Json.decodeFromString(genHandle(init_pubkey_kyber, init_pubkey_curve, init_pubkey_curve_pfs_2, init_pubkey_kyber_for_salt, init_pubkey_curve_for_salt, name, mdc))
        if(libraryResponse.status != "ok") return err(libraryResponse.status)
        return ok(libraryResponse)
    }
    ...
    fun mAcceptInitRequest(own_seckey_sig: String, own_pubkey_sig: String, remote_pubkey_kyber: String, own_pfs_key: String, pfs_salt: String, id: String, mdc_seed: String): Result<AcceptInitRequest, String> {
        val libraryResponse: AcceptInitRequest = Json.decodeFromString(acceptInitRequest(own_seckey_sig, own_pubkey_sig, remote_pubkey_kyber, own_pfs_key, pfs_salt, id, mdc_seed))
        if(libraryResponse.status != "ok") return err(libraryResponse.status)
        return ok(libraryResponse)
    }
